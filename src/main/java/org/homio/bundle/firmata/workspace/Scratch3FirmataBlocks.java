...
import org.homio.bundle.firmata.provider.command.FirmataGetTimeValueCommand;
...
public class Scratch3FirmataBlocks extends Scratch3FirmataBaseBlock {
  ...
  public Scratch3FirmataBlocks(EntityContext entityContext, FirmataBundleEntrypoint firmataBundleEntrypoint,
    ...
    super("#C9BF43", entityContext, firmataBundleEntrypoint, null);
  ...
  private void whenDeviceReady(WorkspaceBlock workspaceBlock) {
    workspaceBlock.handleNextOptional(next -> {
      ...
      if (entity.getJoined() == Status.ONLINE) {
        ...
        BroadcastLock readyLock = workspaceBlock.getBroadcastLockManager().getOrCreateLock(workspaceBlock, "firmata-ready-" + entity.getTarget());
  ...
  private void whenPinChangedHandler(WorkspaceBlock workspaceBlock, Function<Object, Boolean> checkFn) {
    workspaceBlock.handleNextOptional(next -> {
      ...
      execute(workspaceBlock, true, entity -> {
        ...
        BroadcastLock lock = workspaceBlock.getBroadcastLockManager().getOrCreateLock(workspaceBlock, entity.getTarget() + "-pin-" + pin.getIndex());
