...
public class Scratch3FirmataBlocks extends Scratch3FirmataBaseBlock {
  ...
  public Scratch3FirmataBlocks(Context context, FirmataEntrypoint firmataEntrypoint,
    ...
    super("#C9BF43", context, firmataEntrypoint, null);
    ...
    this.menuPinServo = menuServer("pinMenuServo", REST_PIN + Pin.Mode.SERVO, "Servo pin").setDependency(this.firmataIdMenu);
  ...
  private void whenDeviceReady(WorkspaceBlock workspaceBlock) {
    workspaceBlock.handleNextOptional(next -> {
      ...
      FirmataBaseEntity entity = context.db().get(firmataId);
      ...
      if (entity.getJoined() == Status.ONLINE) {
        ...
        var readyLock = workspaceBlock.getLockManager().getLock(workspaceBlock, "firmata-ready-" + entity.getTarget());
  ...
  private void whenPinChangedHandler(WorkspaceBlock workspaceBlock, Function<Object, Boolean> checkFn) {
    workspaceBlock.handleNextOptional(next -> {
      ...
      execute(workspaceBlock, true, entity -> {
        ...
        var lock = workspaceBlock.getLockManager().getLock(workspaceBlock, entity.getTarget() + "-pin-" + pin.getIndex());
