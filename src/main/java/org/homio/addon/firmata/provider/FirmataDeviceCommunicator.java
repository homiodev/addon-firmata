...
public abstract class FirmataDeviceCommunicator<T extends FirmataBaseEntity<T>> extends Consumer<Event>
  ...
  public final String restart() {
    ...
    try {
      ...
      FirmataWatchdog watchdog = new FirmataWatchdog(
        ...
        this.context.event().fireDeviceStatus("firmata", entity);
      });
  ...
  public void onStart(IOEvent event) {
    log.info("Firmata device: {} communication started", entity.getTitle());
  ...
  public void onStop(IOEvent event) {
    log.info("Firmata device: {} communication stopped", entity.getTitle());
    entity.setStatusError("Communication stopped");
    context.event().fireDeviceStatus("firmata", entity);
  }
  ...
  private void entityUpdated(T entity, boolean remove) {
    if (remove) {
      this.destroy();
      this.context.event().removeEvents("firmata-status-" + entity.getEntityID());
    } else {
      this.entity = entity;
    }
  }
